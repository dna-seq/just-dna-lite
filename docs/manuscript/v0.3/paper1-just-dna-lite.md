...
We present Just-DNA-Lite, an open-source, local-first modular platform for personal genome analysis designed for research and educational use, combining two capabilities: variant annotation against curated variant-trait databases, and polygenic risk score (PRS) computation. Just-DNA-Lite runs entirely on a personal computer, processes a whole-genome VCF in under 40 seconds — a 172-fold speedup over the earlier OakVar-based Generation I system on the same hardware — and produces annotated reports across user-selected modules. Annotation knowledge is packaged as modules that are data rather than code — a table of variant-trait associations and a short metadata file — which can be shared, versioned, and installed from public repositories or a dedicated community catalogue, so the annotation catalogue can grow with the published literature without anyone writing software.

The platform ships with expert-curated default modules covering several trait domains (longevity-associated variants, cardiovascular-related variants, lipid metabolism, VO2 max, and athletic performance) as worked examples of the module format, alongside over 5,000 polygenic risk scores from the PGS Catalog computed via the just-prs library with Pearson r = 0.9999 concordance against PLINK2 and a 5.7–12.3× speedup. Because a growing share of analysis happens inside AI assistants rather than browsers, the same scoring operations are also released as `just-prs-mcp`, a locally-run Model Context Protocol server that exposes them as typed tool calls — so an assistant invokes the benchmarked engine instead of improvising a scoring script, and the match rates, model quality, and reference population travel with every result. Every entry point returns numerically identical scores from one engine. Extending the module catalogue with AI-authored modules is the subject of a companion paper and is referenced here only as availability.
...
### **3.2.1 Access beyond the web interface: `just-prs-mcp`**

Not every user works in a browser. A growing number of researchers explore data inside AI coding assistants, and asking such an assistant to "score this genome" ordinarily produces improvised code: the model writes its own scoring loop, picks its own reference panel, and reports a number with no record of which PGS models were used or how many variants matched. We therefore release the PRS engine's operations as `just-prs-mcp`, a Model Context Protocol server that turns them into typed tool calls the assistant invokes rather than reimplements. It is distributed on PyPI, runs locally with `uvx` and no API key, and is listed in the BioContextAI Registry. The scoring, catalog, and reference-panel logic stays in just-prs; this layer contributes the tool schemas, the packaging and plugin manifests, the prompts, and a bundled trait-interpretation skill.

The exposed operations cover the workflow the web interface offers: searching the PGS Catalog and inspecting model metadata, detecting a VCF's genome build and ancestry, normalizing VCF or consumer-array input, computing scores for one PGS ID or for every model associated with a trait, comparing a result against a 1000 Genomes superpopulation, placing it in absolute-risk context, and assessing result quality. Sample lists are the unit of input, so one genome is a one-element list and a two-person comparison uses the same tools with a longer one. Each call returns structured results together with a link to the same interactive HTML report the web application produces, so the assistant hands the user an artifact to open rather than pasting a number into the conversation.

Two design choices follow directly from the platform's principles. **Privacy stays structural:** the server runs on the user's own machine and the genome is never uploaded, so routing PRS through an assistant does not weaken the guarantee that Section 2 establishes for the web application. **Transparency survives the interface:** the tools return PGS IDs, variant-match rates, model quality tiers, the reference population, percentiles, and the agreement or disagreement among models for the same trait, so the evidence a reader needs in order to discount a score travels with the score. This is the same commitment as Section 6.3, expressed through a tool schema instead of a screen — a conversational interface is otherwise the easiest place for a single confident number to lose its caveats.

Regardless of entry point — web application, Python library, command line, standalone PRS web app, or MCP client — results are numerically identical, because all of them are backed by the identical DuckDB/Polars scoring engine benchmarked in Section 5.2. What differs is only how the request arrives and how the result is presented. Note that the assistant here *consumes* validated operations and computes nothing itself; the distinct question of assistants *authoring* new annotation content is the companion paper's subject, not this section's.
...
This paper deliberately treats AI-assisted creation as a platform feature and makes no claim here about its accuracy, its effect on hallucination, or its performance relative to a baseline. **The agentic layer is the companion paper's subject, not this one's.** That paper — *Creating and Sharing Genomic Annotation Modules with AI: The Just-DNA Ecosystem* (in review) — covers the authoring workflow and the agents that carry it out, exposed as a plugin inside AI coding assistants such as Claude Code and Codex; the schema, identifier, and citation checks every authored row must pass before compilation; the local, test-registry, and public-catalog publication routes; and a scoring protocol for variant recall, citation identity, and effect-direction agreement. Neither paper claims that one model, host, or agent arrangement outperforms another: the companion paper's scored runs use one model through one host and are reported as such, so a comparative claim would have no evidence behind it in either place.
...
Just-DNA-Lite delivers on a simple core value: people should be able to explore their own genomes — transparently and privately. It is a self-contained local web application that annotates a whole genome and computes polygenic risk scores in tens of seconds, built on a no-code module system in which anyone can turn a YAML file and a variant table into an annotation module, so the annotation catalogue can grow with the literature. These capabilities rest on a foundation that matches the project's philosophy: all computation is local and the code is open-source (AGPL v3), so privacy is structural rather than promised; the platform shows the full picture rather than a curated verdict; and the performance holds up, with whole-genome annotation in tens of seconds and PRS computation validated at Pearson r = 0.9999 against PLINK2 while running several times faster and using a fraction of the memory. The same engines are reachable from AI assistants as typed tool calls, so a conversational entry point returns the benchmarked engine's numbers together with the evidence needed to judge them, and the catalogue is additionally extensible with AI-authored modules through the companion work described separately. Just-DNA-Lite is a research and educational tool, not a clinical pipeline, and it is explicit about that boundary.
