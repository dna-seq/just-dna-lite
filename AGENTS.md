...
- GRCh38 VCF files remain the primary fully supported path. GRCh37/hg19 liftover support has been added, consumer microarray support is experimental and lower-coverage, and T2T should still be treated as not fully supported unless current code/tests show otherwise. VCF normalization renames `start` to `pos`; PRS computation must account for this, runs in Reflex rather than Dagster, and must clear/rebuild `prs_results_rows`, `prs_results_columns`, and `prs_results_column_groups` after updating `prs_results`.
