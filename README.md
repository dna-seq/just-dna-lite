...
A small set of genomic findings do have solid clinical evidence: pathogenic BRCA1/2 variants, monogenic conditions like Huntington's or familial hypercholesterolemia, pharmacogenomic variants like CYP2C19 or HLA-B*57:01. For these, someone ran prospective studies and proved that knowing the result changes outcomes. That's the clinical-grade bar. Most complex-trait associations haven't cleared it. The modules and PRS scores here are science-grade: findings from real published research, but probabilistic, population-level, and **not clinically validated**.

In a true clinical setting, a genomic finding isn't trusted from a single exploratory sequencing run. If a potential risk variant is found, the standard workflow requires orthogonal validation. This means running a second, targeted test using highly accurate methods like Sanger sequencing, quantitative PCR (qPCR), or clinical-grade microarray panels in a certified laboratory. Only after this rigorous analytical validation, paired with a genetic counselor's clinical interpretation, can a finding be used for medical decisions.
